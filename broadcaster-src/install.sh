...
if [ ! -f .env ]; then
	...
	curl -sf "http://$overseer_host:3200/broadcaster-verify" > /dev/null
	...
	if [[ $overseer_verify_code -eq 6 ]]; then
		...
		echo "This device's IP must be registered in Overseer's config.json"
		echo "or its recordings will be rejected! Ensure this device is using"
		echo "a static IP and you're accessing using the network interface"
		echo "that you were expecting."
		echo ""
	...
	echo "OVERSEER_SERVER=$overseer_host" > .env
fi
